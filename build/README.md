...
2. **`PRAGMA key` wiring** — patch Fossil's `db_open` (in `src/db.c`) to issue `PRAGMA key = '<key>'` after `sqlite3_open_v2`. The key-source UX (env var, prompt, keyfile) is a `docs/threat-model.md` decision, not yet made. The patch lives at `build/patches/fossil-db-key.patch` (to be written).
