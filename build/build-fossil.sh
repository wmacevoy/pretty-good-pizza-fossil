...
# to issue `sqlite3_exec(db, "PRAGMA key = '<key>'", ...)` immediately after
# sqlite3_open_v2(). The key source (env var? prompt? keyfile?) is a UX
# decision pinned in docs/threat-model.md (to be written). Apply the patch
# from build/patches/fossil-db-key.patch once that decision is made.
#
